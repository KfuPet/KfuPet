...
> 日志管道**常开**：不受「开发者模式」开关影响，程序启动时（单实例检查通过后）即开始监听，退出时关闭。
> 开发者模式只控制 `KfuPet.Skeleton` 命令管道。
>
> 日志同时异步落盘到 `%AppData%\KfuPet\Logs\KfuPet-yyyyMMdd-HHmmss.log`（**每次启动一个文件**，启动时清理旧文件，
> 目录内最多保留 4 份），设置窗口「关于」页的「打开日志」按钮可直接打开该目录并选中本次运行的日志文件。
...
> **同时只支持一个客户端**：服务端同一时刻只服务一个连接，第二个客户端要等前一个断开后才能连上。
> 在已有连接占用期间发起 `ConnectAsync` 会等到 5 秒连接超时并抛出异常。
...
> 连接建立后，服务端会先补发最近 1000 条历史日志，再实时推送新日志；历史日志与实时日志不会重复或乱序。

> 单条日志内容超过 4096 个字符时会被截断，并在末尾追加 `…（日志过长已截断）`。

> 客户端读取速度过慢时，待发送队列最多积压 1000 条，超出后丢弃最旧的日志（不会阻塞 KfuPet 主程序）。
